...
"""
论文排版优化器 v2.0 - 合并版
合并了 main.py 和 main_refactored.py 的所有功能

启动方式:
    python3 main_merged.py
    或
    python3 launcher.py (需要修改指向)
"""
...
class PaperFormatterMainWindow(QMainWindow):
    ...
    def init_ui(self):
        """初始化用户界面"""
        self.setWindowTitle('论文排版优化器 v2.0 - 合并版')
        ...
        self.section_tab = self.create_section_tab()
        ...
        self.main_tabs.addTab(self.section_tab, '📝 章节格式化')
    ...
    def create_home_tab(self) -> QWidget:
        ...
        # 上传模板按钮
        upload_template_btn = QPushButton('📤 上传模板')
        ...
        # 封面配置按钮
        cover_config_btn = QPushButton('📘 封面配置')
    ...
    def create_format_tab(self) -> QWidget:
        ...
        # 开始按钮
        start_btn = QPushButton('🚀 开始格式化')
        start_btn.setMinimumHeight(50)
        start_btn.setStyleSheet('background-color: #4caf50; color: white; font-size: 14px; font-weight: bold;')
    ...
    def create_reference_tab(self) -> QWidget:
        ...
        format_ref_btn = QPushButton('格式化参考文献')
    ...
    def create_section_tab(self) -> QWidget:
        """创建章节格式化标签"""
        widget = QWidget()
        layout = QVBoxLayout(widget)
        
        section_group = QGroupBox('📝 章节格式化')
        section_layout = QVBoxLayout(section_group)
        
        info_label = QLabel('选择文档中的章节进行格式化')
        info_label.setStyleSheet('color: #666; font-size: 13px;')
        section_layout.addWidget(info_label)
        
        # 章节列表
        self.section_list = QListWidget()
        self.section_list.setMaximumHeight(300)
        section_layout.addWidget(self.section_list)
        
        # 格式化按钮
        format_section_btn = QPushButton('格式化选中章节')
        format_section_btn.clicked.connect(self.format_selected_section)
        section_layout.addWidget(format_section_btn)
    ...
    def on_help(self):
        """帮助"""
        help_path = os.path.join(os.path.dirname(__file__), '快速入门.md')
    ...
    def format_selected_section(self):
        """格式化选中章节"""
        QMessageBox.information(self, '提示', '章节格式化功能开发中...')
